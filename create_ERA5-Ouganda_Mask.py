...
ds = xr.open_mfdataset('ERA5_T2m_1h_197901_sfc.nc')
lat_bnd = [ 10 , -5 ]
lon_bnd = [ 25 , 40 ]
...
mask.to_netcdf('./mask_ERA5_by_countries.nc')

mask_ouganda = mask.where(mask==758)/mask.where(mask==758)
mask_ouganda.to_netcdf('./mask_ERA5_Ouganda.nc')
...
mm = ax.contourf(mask_ouganda.longitude.values,\
                       mask_ouganda.latitude.values,\
                       mask_ouganda.values ,\
...
plt.savefig('OUGANDA_ERA5_Mask.png', bbox_inches='tight', pad_inches=0.1)
plt.show()   
